        ...
        var ready = function () {
            try {
                ...
                if (JotForm.showHIPAABadge) {
                    ...
                    hipaaWrapper.setAttribute('href', 'https://www.jotform.com/hipaa/');
                ...
                if (JotForm.showA11yBadge) {
                    ...
                    A11yWrapper.setAttribute('href', 'https://www.jotform.com/accessible-forms/');
                ...
                if (JotForm.newDefaultTheme && (JotForm.showHIPAABadge || JotForm.showA11yBadge)) {
                    var allSubmitButtons = document.querySelectorAll('.form-submit-button');
                    if (allSubmitButtons) {
                        for (var i=0; i<allSubmitButtons.length; i++) {
                            allSubmitButtons[i].classList.add('with-badge');
                            allSubmitButtons[i].parentElement.classList.add('with-badge-wrapper');
                        }
                    }
                }
        ...
        switch (resultFieldType) {
            case "inline":
                if (result.indexOf('|') > -1) {
                    ...
                    var TIMESTAMP_OF_2019 = 1546300000000;
                    var selector = Number(fieldId) > TIMESTAMP_OF_2019 ? '#id_' + qid + ' input[id*=' + fieldId + '-]' : '#id_' + qid + ' input[id$=-' + fieldId + ']';
                    var resultFields = $$(selector).filter(function(el) {return JotForm.isVisible(el);});
                    var hasCombinedField = false;
                    Object.keys(combinedObject).forEach(function(id) {
                        var selector = Number(fieldId) > TIMESTAMP_OF_2019 ? '#id_' + qid + ' input[id*=' + fieldId + '-' + id + ']' : '#id_' + qid + ' input[id$=' + id + '-' + fieldId + ']'
   ...
   */
    ...
    autoNext: function(id) {
        ...
        prev.observe("change", function(e) {
            ...
            if (e.target.hasClassName('form-line') && e.target.getAttribute('data-type') === 'control_radio') { return; }
                        ...
                        var checkValues = inputs.map(function (e) {
                            ...
                            if (innerVals.include('required') && JotForm.isVisible(e)) {
                                if (JotForm.getContainer(e).getAttribute('data-type') === 'control_inline' && ['radio','checkbox'].indexOf(e.readAttribute('type')) > -1) {
                                    ...
                                    var baseName = inputType === 'checkbox' ? e.name.substr(0, e.name.indexOf('-')) : e.name; // name is like q{qid}_{qname}[{fieldID}-{blotIndex}]
